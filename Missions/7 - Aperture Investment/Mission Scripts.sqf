...

{
		...
		if (side player != sideLogic) then {
			...
			[ 
				[ 
					...
					["textbold","[Breaking News] French Troops Surrender During NATO Training Exercises, Blames ‘Old Habits’!"], 
		...
		if (side player != sideLogic) then {
			...
			[ 
				[ 
					...
					["text", "'We weren't even halfway through the fake battle!' complained Sergeant Zhukov of the American contingent, 'One minute they were yelling ‘Vive la France!’ and the next, they were running faster than I’ve ever seen anyone move without a German shepherd behind them.'"],
	...
	player createDiaryRecord ["diary", ["AAN News Articles", "<execute expression='[] call ROOT_fnc_myArticle2'>French Troops Surrender During Training!</execute>"]];
