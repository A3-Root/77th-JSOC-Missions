...
['M9_tmpREfnc_svmspc'] call ROOT_fnc_REinit2_V3;
waitUntil {!isNil 'RE2_M9_tmpREfnc_svmspc'};

if (isnil 'ROOT_rocketDebugMode') then {ROOT_rocketDebugMode = 0;};
...
ROOT_fnc_rocketAscent = {
    ...
    0 = [_this] spawn {
        ...
        (_rocketObj_stage_01 getVariable ['enginesFiring', false]))} do {
            ...
            [[_rocketObj_stage_01, _vel], 'RE2_M9_tmpREfnc_svmspc', 0] call ROOT_fnc_RE2_V3;
...
//  \$$$$$$$$ \$$   \$$ \$$   \$$ \$$$$$$$ 
	...
	0 = [] spawn {
		...
		if (isNil 'ROOT_fnc_cleanupRockets') then {
			ROOT_fnc_cleanupRockets = {
				...
				if (_deleteCount_comps > 0) then {
					missionNamespace setVariable ['ROOT_rocketComps', _newRocketCompArray, true];
		...
		if (isNil 'ROOT_fnc_addObjectToGameMaster') then {
			ROOT_fnc_addObjectToGameMaster = {
				private _obj = _this;
		...
		ROOT_fnc_rocketViewDistance = {
			if (isNil 'ROOT_ogVD') then {
				...
				systemChat format ["ROOT_ogVD saved as %1", ROOT_ogVD];
				missionNamespace setVariable ['ROOT_ogOVD', _ogOVD, true];
				systemChat format ["ROOT_ogOVD saved as %1", ROOT_ogOVD];
			};
		...
		ROOT_fnc_initRocketSequence_landing = {
			params ["_landingposition", "_addDelay"];

			uiSleep _addDelay;
			
			if ((call ROOT_fnc_countRockets > 20) && (ROOT_rocketDebugMode != 1)) exitWith {
			...
			private _rocketComp = M9_sShipLandPos call ROOT_fnc_rocketAssembly_landing;
			...
			while {(getPosATL _rocket # 2) > 1000} do {
				...
				[[_rocket, [0,0,-277.778]], 'RE2_M9_tmpREfnc_svmspc', 0] call ROOT_fnc_RE2_V3;
			...
			_rocket call ROOT_fnc_rocketSpin;
			_rocket spawn ROOT_fnc_rocketCleanup;
		};
 ...
 * Made exclusively for use in mission by 77th JSOC
 * Can be executed in any valid order
 * 
 * 
 * CREATING / SPAWNING A ROCKET
 * 
 * Arguments:
 * 0: Spawn Position <ARRAY>
 * 1: Sleep Delay <NUMBER>
 *
 * Return Value:
 * None
 *
 * Example:
 * [[5091.82,6482.27,0], 5] spawn ROOT_fnc_rocketAssembly;
 *
 ...
 *
 * Example:
 * [] spawn ROOT_fnc_launchAllRockets;
 *
 * Public: No (unless Remote-Executed intentionally)
 * 
 * 
