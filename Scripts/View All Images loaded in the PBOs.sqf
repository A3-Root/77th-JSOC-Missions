		...
		case "create": {
			...
			for [{_i = 0}, {_i < _boxesY}, {_i = _i + 1}] do {
				...
				for [{_j = 0}, {_j < _boxesX}, {_j = _j + 1}] do {
					...
					_ctrlBox setVariable ["ctrls", [_ctrlTextBG, _ctrlPicture, _ctrlButton]];
			...
			if (_items isEqualTo []) then {
				...
				for [{_i = 0}, {_i < _iconsPerPage}, {_i = _i + 1}] do { 
					...
					if (_idx < _n) then {
						...
						(_ctrlBox getVariable ["ctrls", []]) params ["", "_ctrlPicture", "_ctrlButton"];
                        ...
                        _ctrlButton ctrlSetTooltip _img;
